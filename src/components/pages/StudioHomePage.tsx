...
export function StudioHomePage() {
  ...
  // Route-derived active tab (so BottomDock highlights correctly)
  ...
  const handleNewVideoProject = () => {
    const p = createProject(`Video Project ${projects.length + 1}`);
    nav(`/editor/${p.id}`);
  };

  const handleNewImageProject = () => {
    const p = createProject(`Image Project ${projects.length + 1}`);
    // if you eventually have image editor route:
    // nav(`/image/${p.id}`);
    nav(`/editor/${p.id}`); // keep simple for now
  };

  const handleOpenProject = (id: string) => nav(`/editor/${id}`);

  const handleSyncNow = async () => {
    console.log("sync now (todo)");
  };

  // newest first (your store already prepends, but keep it stable)
...
function tabToRoute(t: TabKey) {
  switch (t) {
    ...
    case "image":
      return "/image";
    case "editor":
      return "/editor"; // optional: you can route to last project, etc.
    case "feed":
      return "/feed";
    case "profile":
      return "/profile";
